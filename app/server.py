...
GEMINI_MODEL_NAME = "gemini-1.5-flash" # Modello Gemini da utilizzare

# === GESTIONE MODELLI ===
face_analyzer, face_swapper, face_restorer, yolo_parser, sam_predictor, pipe, canny_detector = (None,) * 7

def release_vram():
    """Rilascia la memoria VRAM della GPU."""
...
def normalize_image(img: Image.Image, max_dim=MAX_IMAGE_DIMENSION) -> Image.Image:
    """Ridimensiona un'immagine se supera le dimensioni massime, mantenendo l'aspect ratio."""
    ...
    if width > max_dim or height > max_dim:
        ...
        # Assicura che le dimensioni siano divisibili per 8
...
def make_mask(pil_img, parts_to_mask, conf_threshold=0.20):
    """
    Crea una maschera binaria per le parti specificate dell'immagine.
    Utilizza prima i poligoni di segmentazione di YOLO. Se fallisce, usa SAM con i bounding box.
    """
    if sam_predictor is None or yolo_parser is None:
        print(" [ATTENZIONE] Parser YOLO o SAM non caricati. Impossibile creare la maschera.")
        return None
    ...
    if not target_idx:
        print(f" [ATTENZIONE] Nessuna delle parti specificate {parts_to_mask} trovata nell'immagine.")
        return None
        
    final_mask_np = None

    # Metodo 1: Prova a creare la maschera usando i poligoni di segmentazione di YOLO
    ...
    # Metodo 2 (Fallback): Se il Metodo 1 fallisce, usa SAM con i bounding box
    if final_mask_np is None:
        print(" [INFO] Maschera YOLO non creata, si passa a SAM...")
        detected_boxes = [b.xyxy[0].cpu().numpy() for b in res.boxes if int(b.cls.item()) in target_idx and b.conf.item() > conf_threshold]
        if not detected_boxes:
            print(" [ATTENZIONE] Nessun bounding box valido trovato per SAM.")
            return None
        ...
        if masks is None:
            print(" [ERRORE] SAM non ha restituito maschere.")
            return None

        # Scegli la maschera con il punteggio più alto
    ...
    if final_mask_np is None:
        print(" [ERRORE] Nessun metodo è riuscito a creare una maschera.")
        return None
    
    # Operazioni morfologiche per pulire e dilatare la maschera
